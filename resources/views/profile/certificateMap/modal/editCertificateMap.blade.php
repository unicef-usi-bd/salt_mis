<div class="col-md-12">
    ...
    <form action="{{ url('/certificate-map/'.$editCertificate->CERTIFICATE_MAP_ID) }}" method="post" class="form-horizontal" enctype="multipart/form-data" role="form">
        @csrf
        @method('PUT')
        <div class="col-md-12">
                    ...
                    <span class="block input-icon input-icon-right" style="width: 365px; margin-left: 5px;">
                        <select class="form-control chosen-select " name="CERTIFICATE_TYPE_ID" data-placeholder="Select or search data">
                            ...
                            @foreach($certificate as $row)
                                <option value="{{$row->LOOKUPCHD_ID}}" @if($editCertificate->CERTIFICATE_TYPE_ID == $row->LOOKUPCHD_ID) selected @endif> {{$row->LOOKUPCHD_NAME}}</option>
                            @endforeach
                        </select>
                    </span>
                </div>
                    ...
                    <span class="block input-icon input-icon-right" style="width: 365px; margin-left: 5px;">
                        <select class="form-control chosen-select " name="ISSURE_ID" data-placeholder="Select or search data">
                            ...
                            @foreach($issueBy as $row)
                                <option value="{{$row->LOOKUPCHD_ID}}" @if($editCertificate->ISSURE_ID == $row->LOOKUPCHD_ID) selected @endif> {{$row->LOOKUPCHD_NAME}}</option>
                            @endforeach
                        </select>
                    </span>
                </div>
