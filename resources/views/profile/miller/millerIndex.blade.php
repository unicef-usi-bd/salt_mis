    ...
    <div class="page-header">
        <h1>
            ...
            <small>
                <i class="ace-icon fa fa-angle-double-right"></i>
                {{--{{ trans('soeReport.report_dashboard') }}--}}
                Miller Profile For Approve After Edit by miller
            </small>
        </h1>
    </div><!-- /.page-header -->
        ...
        <div class="col-md-12" style="width: 1040px;">
            <div class="col-sm-12">
                ...
                <table class="table table-striped table-bordered table-hover gridTable" title="{{ trans('module.module_list') }}">
                    ...
                    <tbody>
                    <?php $sl=0;?>
                    @foreach( $approvalMillList as $row)
                        <?php
                        $activeFlg = DB::selectOne(DB::raw("SELECT ACTIVE_FLG FROM tem_ssm_mill_info WHERE MILL_ID_TEM = '$row->MILL_ID_TEM' "));
                        ?>
                        <tr>
                            <td class="center" >  {{ ++$sl }}</td>
                            <td> {{ $row->MILL_NAME }} </td>
