        ...
        <div class="col-md-12">
            <div class="alert alert-info entrepreneur_msg"></div>
                <form id="entrepreneurId"  class="form-horizontal myform" role="form">
                    ...
                    @php
                        $milltype =  DB::select(DB::raw("select OWNER_TYPE_ID, MILL_ID from ssm_mill_info where MILL_ID = $millerInfoId"));
                    @endphp
                        ...
                        <table id="enterprenurInfoTableID" class="table table-bordered fundAllocation" style="margin-top: 64px;">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:30px;" class="addButton @if($editMillData->OWNER_TYPE_ID == 12) hidden @endif"><span class="btn btn-primary btn-sm pull-right rowAddEntrepreneur"><i class="fa fa-plus"></i></span></th>

                                </tr>
                            </thead>
                            <tbody class="tableEntrepreneur">
...
<script>
    ...
    $(document).ready(function() {
        $(document).on('click', '.rowAddEntrepreneur', function () {
            let getTr = $('tr.removableRow:first');
...
//            For Ignore array Conflict
        ...
        $(document).on("click", "span.EntrowRemove ", function () {
            let count = $('.tableEntrepreneur tr').length - 1;
            if(count > 1) {
        ...
        $(document).on('change','.DIVISION_ID',function () {
            ...
            let divisionId = $(this).val(); //alert(divisionId); //exit();
            let option = '<option value="">Select District</option>';
            let url  = $(this).attr('url');
            let url = url+'/'+divisionId;
            $.ajax({
                type : "get",
                url  : url,
    ...
    function checkValidation(selector) {
        let thisInput = selector.closest('.block');
        let mobile = $('.MOBILE_1').val();
        let mobile2 = $('.MOBILE_2').val();
        let status = true;
    ...
    $(document).on('keyup', '.MOBILE_1', function () {
        let mobile =$(this).val();
        let status = checkMobileDigit(mobile);
        if(status!==true){
    ...
    $(document).on('keyup', '.MOBILE_2', function () {
        let mobile2 =$(this).val();
        let status = checkMobileDigit(mobile2);
        if(status!==true){
