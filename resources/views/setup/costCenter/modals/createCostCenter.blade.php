<div class="col-md-12">
    <!-- PAGE CONTENT BEGINS -->
    <form action="{{ url('/cost-center') }}" method="post" class="form-horizontal checkValidation" role="form" enctype="multipart/form-data">
        @csrf
            ...
            <span class="block input-icon input-icon-right">
                <select class="form-control addInputField" id="cost_center_type" name="cost_center_type">
                    ...
                    @foreach($costTypes as $costType)
                        <option value="{{ $costType->cost_center_type_id }}"> {{ $costType->cost_center_type_name }} </option>
                    @endforeach
                </select>
            </span>
            </div>
            ...
            <span class="block input-icon input-icon-right">
                <select class="form-control cost_center_status_type" name="cost_center_status_type">
                    ...
                    @foreach($upzilaType as $upzila)
                        <option value="{{ $upzila->lookup_group_data_id }}" @if($upzila->lookup_group_data_id==48) selected @endif>{{ $upzila->group_data_name }}</option>
                    @endforeach
                </select>
            </span>
            </div>
